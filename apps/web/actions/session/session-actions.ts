'use server'
import { GetReturnTypeSession, InputTypeSession, CreateReturnTypeSession, GetReturnTypeSingleSession, VerifyTypeSession, GetSessionMessages, GetSessionQuestions } from "./types";
...
export const getSessionQuestions = async (room_code: string, limit?: number, offset?: number): Promise<GetSessionQuestions> => {
    try {
        const room = await db.room.findUnique({
            where:{
                room_code: room_code
        ...
        const paginatedMessages = annotate.slice(offset, offset + limit);
        return { data: paginatedMessages.reverse(), count: annotate.length, message: 'Session questions fetched successfully.'}
    }  catch(err) {
        console.log(err);
        return { error: 'Failed to fetch session messages.' };
    }
